...
public class Select extends Element {
    private Logger logger = Logger.getLogger(Select.class);
    ...
    public void selectByIndex(int index){
        if(isExist()){
            ...
            throw new NoSuchElementException("["+RuntimeMethod.getName()+"]"+"["+this.getId()+"]通过索引进行选择的时候失败了，这个元素不存在，没有被找到！");
        }
    }
    ...
    public void selectByValue(String value){
        if(isExist()){
            ...
            throw new NoSuchElementException("["+RuntimeMethod.getName()+"]"+"["+this.getId()+"]通过内容查找元素的时候失败了！这个元素不存在，没有被找到！");
        }
    ...
    public void selectByVisiableText(String text){
        if(isExist()){
            ...
            throw new NoSuchElementException("["+RuntimeMethod.getName()+"]"+"["+this.getId()+"]通过标签之间的内容来进行查找的时候失败了，这个元素没有被找到！");
        }
    ...
    public int getSelectedIndex(){
        ...
        if(isExist()){
            ...
            throw new NoSuchElementException("["+RuntimeMethod.getName()+"]"+"["+this.getId()+"]获取当前项的索引值的时候出现了错误！可能的原因是这个元素不存在");
        }
    ...
    public String getSelectedValue(){
        ...
        if(isExist()){
            ...
            throw new NoSuchElementException("["+RuntimeMethod.getName()+"]"+"["+this.getId()+"]获取当前项的内容的时候出现了错误！可能的原因是这个元素定位错误没有找到！");
        }
    }
    ...
    public String[] getSelectedValues(){
        ...
        if(isExist()){
            ...
            throw new NoSuchElementException("["+RuntimeMethod.getName()+"]"+"["+this.getId()+"]获取多个元素的时候失败了！可能的原因是这个元素不存在，没有被找到！");
        }
    }
    ...
    public int[] getSelectedIndexs(){
        ...
        if(isExist()){
            ...
            throw new NoSuchElementException("["+RuntimeMethod.getName()+"]"+"["+this.getId()+"]获取多个索引值的时候失败！可能的原因是这个元素没有找到！");
        }
    }
    /**获得控件里面的大小*/
    public int getComoboBoxSize(){
        ...
        if(isExist()){
            ...
            throw new NoSuchElementException("["+RuntimeMethod.getName()+"]"+"["+this.getId()+"]获取控件大小的时候出现了错误！可能的原因是这个元素不存在");
        }
    }
